...
def home(request):
    ...
    for vehicle in vehicles:
        ...
        if latest_booking and latest_booking.end_time > timezone.now():
            ...
            # Calculate total hours and minutes, accounting for days
            days = remaining_time.days
            hours = remaining_time.seconds // 3600
            minutes = (remaining_time.seconds % 3600) // 60

            # Add the hours from the days into the total hours
            total_hours = days * 24 + hours  # convert days into hours and add the remaining hours
            vehicle_data.append({
                'vehicle': vehicle,
                'vehicle_number': vehicle.vehicle_number,
                'parking_lot': latest_booking.parking_space.parking_lot.name,
                'parking_space': latest_booking.parking_space.spot_number,
                'remaining_time': f"{total_hours} hours, {minutes} minutes",  # Correct format
...
from .forms import LoginForm  # Make sure your LoginForm is correctly imported
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                if not hasattr(user, 'profile'):
                    UserProfile.objects.create(user=user)  # Create profile if it doesn't exist
